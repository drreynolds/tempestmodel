...
void TimestepSchemeARKode::Initialize() {
  ...
  // set nonlin coeff
  //  ierr = ARKodeSetErrorBias(arkode_mem, 2.1);
  //  if (ierr < 0) _EXCEPTION1("ERROR: ARKodeSetErrorBias, ierr = %i",ierr);
  ...
  if (m_fDynamicStepSize) {
    ...
    ierr = ARKodeSetMaxGrowth(arkode_mem, 2.5);
    if (ierr < 0) _EXCEPTION1("ERROR: ARKodeSetMaxGrowth, ierr = %i", ierr);
  } else {

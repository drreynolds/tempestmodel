function [qE,qI,q] = check_ark(cE,cI,AE,AI,bE,bI,tol,reportL,doPlot)
% Usage: [qE,qI,q] = check_ark(cE,cI,AE,AI,bE,bI,tol,reportL,doPlot)
% 
...
if (reportL>0)
   ...
   fprintf('    ERK:  order = %i,  B/A/L stability = %i/%i/%i\n', qE,BsE,AsE,LsE);
   fprintf('    DIRK: order = %i,  B/A/L stability = %i/%i/%i\n', qI,BsI,AsI,LsI);
   fprintf('    ARK:  order = %i\n', q);
end
...
if (doPlot)
   ...
   xlabel('Re(z)')
   ylabel('Im(z)')
   title(sprintf('Stability boundaries for ARK method, order %i',q))
   legend(sprintf('ERK, order %i',qE),sprintf('DIRK, order %i',qI))

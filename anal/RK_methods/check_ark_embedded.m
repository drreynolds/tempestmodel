function [qE,qI,q,pE,pI,p] = check_ark_embedded(cE,cI,AE,AI,bE,bI,dE,dI,tol,reportL,doPlot)
% Usage: [qE,qI,q,pE,pI,p] = check_ark_embedded(cE,cI,AE,AI,bE,bI,dE,dI,tol,reportL,doPlot)
% 
...
BE = [cE, AE; 0, bE; 0, dE];
[qE,pE,BsE,AsE,LsE] = check_butcher(BE);
BI = [cI, AI; 0, bI; 0, dI];
[qI,pI,BsI,AsI,LsI] = check_butcher(BI);
...
if (reportL>0)
   ...
   fprintf('    ERK:   order = %i,  embed order = %i,  B/A/L stability = %i/%i/%i\n', ...
           qE, pE, BsE, AsE, LsE);
   fprintf('    DIRK:  order = %i,  embed order = %i,  B/A/L stability = %i/%i/%i\n', ...
           qI, pI, BsI, AsI, LsI);
   fprintf('    ARK:   order = %i,  embed order = %i\n', q, p);
end
...
if (doPlot)
   ...
   [X,Y] = stab_region(AE, bE, Theta);     % ERK stability region boundary
   plot(X, Y, 'r-'), hold on
   [X,Y] = stab_region(AE, dE, Theta);     % ERK embedding stability region boundary
   plot(X, Y, 'g-')
   [X,Y] = stab_region(AI, bI, Theta);     % DIRK stability region boundary
   plot(X, Y, 'b-')
   [X,Y] = stab_region(AI, dI, Theta);     % DIRK embedding stability region boundary
   plot(X, Y, 'm-')
   xl = xlim();  yl = ylim();
   plot( linspace(xl(1),xl(2),10), zeros(1,10), 'k:')
   plot( zeros(1,10), linspace(yl(1),yl(2),10), 'k:'), hold off
   xlabel('Re(z)')
   ylabel('Im(z)')
   title(sprintf('Stability boundaries for ARK method, order %i, embed order %i',q,p))
   legend(sprintf('ERK, order %i', qE),...
          sprintf('ERK embedding, order %i', pE),...
          sprintf('DIRK, order %i', qI),...
          sprintf('DIRK embedding, order %i', pI) )
   print('stab_regions.png', '-dpng');
end
